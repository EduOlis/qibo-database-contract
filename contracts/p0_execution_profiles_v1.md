...
- `page_reference` obrigatório, utilizando posição Kindle no formato `loc X–Y`, quando essa informação estiver disponível no artefato convertido.

### Restrição Crítica
Este perfil **não deve ser utilizado** caso a ferramenta de conversão não exponha ou preserve a localização Kindle.
Nesses casos, a ingestão deve falhar explicitamente e ser registrada como erro, sem geração de chunks.
